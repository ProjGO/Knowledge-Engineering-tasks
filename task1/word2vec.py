...
def word2vec_basic(log_dir):
    ...
    def maybe_download(filename, expected_bytes):
        ...
        local_filename = '../datasets/text8.zip'
    ...
    def build_dataset(words, n_words):
        ...
        unk_count = 0
        for word in words:
            index = dictionary.get(word, 0)  # get(key, default=0),dictionary里查不到的都是0,后面将0当作UNK
    ...
    with graph.as_default():
        ...
        # norm = tf.sqrt(tf.reduce_sum(tf.square(nce_weights), 1, keepdims=True))
        normalized_embeddings = embeddings / norm
        print(norm.name)
        print(normalized_embeddings.name)
        # normalized_embeddings = nce_weights / norm
    ...
    try:
        ...
        # plot_with_labels(low_dim_embs, labels, os.path.join(gettempdir(),'tsne0530-1.png'))
        plot_with_labels(low_dim_embs, labels, 'tsne_withoutVo.png')
...
def main(unused_argv):
    ...
    # word2vec_basic(log_dir='D:/ML/Ckpts/KnowledgeEngineering_task1/log')
